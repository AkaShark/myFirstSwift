...
static NSString *CJQiNiuScope = @"camelliaapp";
static NSString *CJQiNiuAccessKey = @"e_jutmoyGMngxSdnBwY4hb0OhSSKfGGhV2q_z1uh";
static NSString *CJQiNiuSecretKey = @"VPnXTwiBcGRmWGW364MD14PyLF5dQC1DU5gdI8Y8";
...
- (void)createToken {
    [self registerWithScope:CJQiNiuScope
...
- (void)uploadData:(NSData *)data key:(NSString *)key
...
- (void)uploadDatas:(NSArray<NSData *> *)datas key:(NSArray<NSString *> *)key
           progress:(UploadProgressHandler)progress
 ...
 allTasksCompletion:(UploadAllTasksCompletion)allTasksCompletion {
    ...
    if (self.index < datas.count) {
        [self uploadData:datas[self.index] key:key[self.index]
        ...
        completion:^(NSError *error, NSString *link, NSInteger index) {
            ...
            self.index++;
            [self uploadDatas:datas key:key
                      progress:progress
