...
class WKWebviewController: UIViewController,WKUIDelegate,WKNavigationDelegate
{
    ...
    lazy var progress:UIProgressView =
        {
            () -> UIProgressView in
            var rect:CGRect = CGRect.init(x: 0, y: 0, width: WIDTH, height: 2.0)
            let tempProgressView = UIProgressView.init(frame: rect)
            tempProgressView.tintColor = webProgressColor
    ...
    {
        self.leftBarButton = UIBarButtonItem.init(image: UIImage.init(named: "返回-3"), style: UIBarButtonItemStyle.plain, target: self, action: #selector(WKWebviewController.selectedToBack))
        ...
        self.leftBarButtonSecond?.imageInsets = UIEdgeInsetsMake(0, -10, 0, 10)
        
        let items = [self.leftBarButton]
